...
│   │   └── system_camera/     # calibrate_view: grab-staged interactive re-derive of deskewed 4:3 screen_roi (manual drag + arrow-key rotate) + manually-dragged arc boxes + auto-swept red HSV band/threshold + test-loop refinement -> system_camera_config.yaml
...
uv run python scripts/diagnostics/system_camera/arc_sweep_replay.py [--from DIR] [--write]   # OFFLINE (no hardware): re-run the red-detection sweep over saved labelled cases (default media_outputs/arc_calib/) -> prints chosen red band + coverage_threshold + per-case ok/WRONG/EXCLUDED-transitional; --write applies a SEPARABLE result to system_camera_config.yaml via write_calibration_values (swept bands/threshold + cases' arcs, screen_roi preserved, .bak, [y/N] confirm)
