...
│   ├── system_camera/         # device layer — arm-mounted USB observation cam (films the Aurora screen): cv2 live preview + record + last-capture still popup; fixed-ROI zoom (roi.py) + imshow_fit aspect-preserving letterbox; manual-focus lock (focus.py probe core + open_capture autofocus/focus, dshow-only)
