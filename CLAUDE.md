...
│   ├── system_camera/         # device layer — arm-mounted USB observation cam (films the Aurora screen): cv2 live preview + record + last-capture still popup; config-driven ROI zoom (roi.py + roi_from_region; screen_roi lives in system_camera_config.yaml) + imshow_fit aspect-preserving letterbox; manual-focus lock (focus.py probe core + open_capture autofocus/focus, dshow-only); arc auto-trigger (arc_detector = HSV red/green alignment-arc classification, auto_trigger = pure lifecycle state machine; WebcamPreview latest_frame snapshot + status overlay); view-calibration math (calibration.py = screen-rect detection + 4:3 ROI candidate + arc-region/HSV-band sampling + ruamel round-trip writer)
...
│   │   └── system_camera/     # calibrate_view: interactive re-derive of screen_roi + arc regions + HSV bands -> system_camera_config.yaml
...
uv run python scripts/demos/grab_auto_trigger_analysis.py [--grade|--no-grade]  # grab_trigger_capture_analysis + CV AUTO trigger: 'm' toggles MANUAL<->AUTO; in AUTO the arc_detector watches the Aurora's two alignment arcs and auto-fires the capture when they go green + hold stable (stable_seconds), re-arming after the arcs LEAVE green (re-align per shot); 'g' grades the patient's shots, 'n' = next patient. SPACE fires only in MANUAL. Arc regions/HSV/timing in data/system_camera_config.yaml (auto_trigger block)
...
- **System-camera preview + DR-grading inference are the computer-vision code so far.** `src/arm101_hand/system_camera/` (cv2 HighGUI window + recording, plus a "last capture" still popup the same thread hosts via `WebcamPreview.show_still`) films the Aurora screen; used by `grab_trigger_capture` + `usb_camera_probe`. The screen ROI now lives in `system_camera_config.yaml` (`screen_roi`, read via `roi_from_region`; `roi.py`'s `AURORA_SCREEN_ROI` is only the fallback default) and zooms the preview + recording + detection onto the screen, and `imshow_fit` letterboxes to preserve aspect ratio because `cv2.WINDOW_KEEPRATIO` is a silent no-op on this wheel's Win32 highgui backend. The `screen_roi`, the two alignment-arc regions, and the red/green HSV bands are all re-derived by `scripts/calibration/system_camera/calibrate_view.py` (interactive white -> red -> green capture + confirm; pure detection math in `system_camera/calibration.py`, comment-preserving ruamel writer) after any resolution or lighting change. Focus is locked to a fixed manual value (`system_camera_config.yaml` `autofocus: false` / `focus: 600`, applied in `open_capture` via `_apply_focus`) so the fixed-geometry Aurora screen stays sharp instead of autofocus hunting/breathing — this **requires `backend: dshow`** (MSMF accepts the focus calls but doesn't drive the VCM); the value was found with `usb_camera_focus_probe.py` (sharpness math in `focus.py`). It needs the full `opencv-python` wheel, so `pyproject.toml` drops lerobot's `opencv-python-headless` pin via `[tool.uv] override-dependencies` — keep that override (a headless-pinning dep re-clobbers `cv2`). The newest CV piece is the **arc auto-trigger**: `arc_detector` classifies the Aurora's two on-screen alignment arcs RED/GREEN/NONE by HSV per-arc-band masking + pixel coverage (no shape detection), and `auto_trigger` is a pure clock-injected lifecycle (wait-green -> stabilize `stable_seconds` -> fire -> cooldown -> wait-clear) that re-arms when the arcs LEAVE green (`require_clear_between`, keyed on not-green rather than red — the screen often goes blank off-eye); both feed `grab_auto_trigger_analysis` (multi-shot per patient, 'g' grades, 'n' next) via `WebcamPreview.latest_frame` + a status overlay, config in the `system_camera_config.yaml` `auto_trigger` block, design/plan in `docs/superpowers/{specs,plans}/2026-06-19-arc-auto-trigger-*`. The vendored `references/computer-vision/` (OpenCV, MediaPipe, GazeTracking) is still untouched, for planned gaze work.
