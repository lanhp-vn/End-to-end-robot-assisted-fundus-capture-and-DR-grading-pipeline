"""Robot-triggered Aurora capture WITH inline diabetic-retinopathy grading.

This is the analysis variant of ``grab_trigger_capture.py`` (which stays capture-only).
It adds Phase-2 DR grading: each patient turn accumulates one or more captured fundus
...
Usage:
  uv run python scripts/demos/grab_trigger_capture_analysis.py [--grade | --no-grade]
"""
        ...
        try:
            while True:
                ...
                if key in ("r", "R"):  # toggle USB-preview recording (separate from the capture cycle)
                    _toggle_recording(preview)
                    continue
                if key in ("g", "G"):  # analyze the current patient turn
                ...
                for f in new:
                    ...
                    if len(turn_shots) >= target:
                        ...
                        print(
                            f"  shot {len(turn_shots)}/{target} captured -- SPACE for another, 'g' to analyze."
                        )
