"""Pydantic schema for ``src/arm101_hand/data/fundus_config.yaml`` (primitive layer).

Operator config for the Optomed Aurora fundus camera over the Pictor Wi-Fi API: connection
ports + timeouts, and capture-loop tuning (hold, new-file wait, paths). Hand-editable; never
written by runtime code (IL-5). Press-depth bounds reuse ``index_toggle`` constants, so they
are deliberately absent here. The arm-mounted USB observation camera is configured separately
in ``system_camera_config.py``.
"""
...
class FundusConnection(BaseModel):
...
class FundusCapture(BaseModel):
    ...
    fundus_dir: str = Field(
        default="media_outputs/fundus_images", description="repo-relative fundus-image save folder"
    )
...
class FundusConfig(BaseModel):
    ...
    connection: FundusConnection = Field(default_factory=FundusConnection)
    capture: FundusCapture = Field(default_factory=FundusCapture)


def load_fundus_config(path: Path) -> FundusConfig:
    """Parse and validate ``fundus_config.yaml``."""
    raw = yaml.safe_load(path.read_text(encoding="utf-8")) or {}
    return FundusConfig.model_validate(raw)
