...
Normalises the chosen Aurora screen ROI (deskewed to an upright 4:3 crop) and tunes the LAB a*
red-coverage cutoff + threshold from the labelled sample frames. Pure numpy + cv2 imgproc -- no
HighGUI window; the one I/O function is ``write_calibration_values`` (ruamel round-trip).
Synthetic-numpy unit-testable, mirroring ``arc_detector.py``. The interactive capture/confirm shell
lives in ``scripts/calibration/system_camera/calibrate_view.py``.

The ROI is chosen interactively (manual drag + arrow-key deskew) in the calibrate_view shell; this
module only normalises the chosen rect to a 4:3 deskewed RoiBox, runs the a* red-detection sweep, and
round-trips the config. Plain opencv-python only (no contrib).
"""
